...
std::string read_file(std::string source)
{
    ...
    if (file.is_open())
    {
        ...
        file.close();
        auto result = string(contents, size);
        delete[] contents;
        return result;
    }
    return "";
...
WinxFileHandle *open_file(std::string file_name, std::string mode)
{
    ...
    else
    {
        ...
        {
            file_handle->out = new std::ofstream(file_name.c_str(), std::ios_base::app);
        }
        else
        {
...
std::string read_file(WinxFileHandle file_handle)
{
    ...
    if (file_handle.in != nullptr)
    {
        raw = std::string((std::istreambuf_iterator<char>(*file_handle.in)), (std::istreambuf_iterator<char>()));
    }
    return raw;
}
...
void debug_print(std::string prefix, std::string message)
{
    ...
    std::cout << "[" << termcolor::bold << termcolor::magenta << prefix << termcolor::reset << "]: " << message
              << std::endl;
}
