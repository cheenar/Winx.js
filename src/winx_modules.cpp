...
MaybeLocal<Module> Winx::Modules::LoadModule(Local<Context> context, string module_name, string module_code)
{
    Isolate *isolate = context->GetIsolate();
    Local<String> source_code = String::NewFromUtf8(isolate, module_code.c_str()).ToLocalChecked();
    Local<String> name = String::NewFromUtf8(isolate, module_name.c_str()).ToLocalChecked();
    // This feels hacky because it is hacky. This is mostly the defaults of
    // ScripOrigin but with the is_module set to true
    ScriptOrigin origin(name.As<Value>(), 0, 0, false, -1, Local<Value>(), false, false, true, Local<Data>());
...
Local<Module> Winx::Modules::CheckModule(Local<Context> context, MaybeLocal<Module> module)
{
    ...
    Maybe<bool> result = checked_module->InstantiateModule(context, Winx::Modules::ResolveModuleCallback);
    if (result.IsNothing())
    {
        // Local<Value> exception = isolate->ThrowException(
...
Local<Value> Winx::Modules::ExecuteModule(Local<Context> context, Local<Module> module, bool nsObject)
{
    Isolate *isolate = context->GetIsolate();
    ...
    if (!module->Evaluate(context).ToLocal(&result))
    {
        Local<Value> exception =
            isolate->ThrowException(String::NewFromUtf8(isolate, "Failed to execute module").ToLocalChecked());
        return exception;
    }
...
MaybeLocal<Promise> Winx::Modules::ResolveDynamicModuleCallback(Local<Context> context,
                                                                ...
                                                                Local<FixedArray> import_assertions)
{
    ...
    Local<Module> mod = Winx::Modules::CheckModule(
        context, Winx::Modules::LoadModule(context, *specifier_string, Winx::Util::read_file(*specifier_string)));
...
MaybeLocal<Module> Winx::Modules::ResolveModuleCallback(Local<Context> context, Local<String> specifier,
                                                        Local<FixedArray> import_assertions, Local<Module> referrer)
{
    ...
    auto modules_folder = Winx::Config::get_winx_flag(WINX_CONFIG_MODULE_FOLDER);
    ...
    return LoadModule(context, *specifier_string, Winx::Util::read_file(modules_folder));
}
